...
from HnYMusic import BOT_NAME, BOT_USERNAME, SUDO_USERS, db_mem
from HnYMusic.Helpers.PyTgCalls import Queues, Fallen
from HnYMusic.Helpers.Database import (add_active_chat, is_active_chat, music_off,
                            music_on)
from HnYMusic.Helpers.Inline import audio_markup, primary_markup
...
async def start_stream(
    ...
    if await is_active_chat(message.chat.id):
        ...
        if file not in db_mem:
            db_mem[file] = {}
        cpl = f"HnYMusic/Cache/{_path_}.png"
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        got_queue.append(to_append)
        final_output = await message.reply_photo(
            photo="HnYMusic/Utilities/Audio.jpeg",
            caption=(
